// ============================================
// SMARTDEBATE - DEBATES LIST PAGE (Updated)
// src/components/pages/DebatesListPage.jsx
// Assignment 04 - Role-Based CRUD Access
// ============================================
...
const DebatesListPage = () => {
  ...
  // Role check — can user edit/delete this debate?
  const canModify = (debate) => {
    if (isAdmin) return true;
    return debate.createdBy === currentUser?.uid;
  };
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-slate-400">Loading debates...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-7">
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm">
            {filtered.length} debates found
            {isAdmin && <span className="ml-2 text-xs px-2 py-0.5 bg-red-600/20 text-red-400 rounded-full border border-red-500/30">🛡️ Admin — Full Access</span>}
          </p>
        </div>
        <Link to="/create"
      ...
      {/* Filters */}
      <div className="glass rounded-2xl p-5 border border-purple-500/20 mb-7">
        ...
        <button onClick={() => { setSearchTerm(""); setFilterTopic(""); setFilterStatus(""); setSortBy(""); }}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((debate) => (
            <div key={debate.id}
              className="glass rounded-xl p-6 border border-purple-500/20 card-hover relative overflow-hidden group">
              ...
              <div className="flex justify-between items-start mb-3">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    ...
                    {debate.trending && (
                      <span className="text-xs px-2 py-0.5 bg-orange-600/30 text-orange-400 rounded-full">🔥</span>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-1.5 text-xs text-slate-400 mb-4 pb-3 border-b border-slate-700/50">
                ...
                <div className="flex items-center gap-1"><i className="fas fa-users text-cyan-400"></i>{debate.participants}</div>
              ...
              {canModify(debate) && (
                <p className="text-xs text-slate-600 mb-2">
                  {isAdmin ? "🛡️ Admin access" : "✏️ Your debate"}
                </p>
              )}
              ...
              <div className="flex gap-2">
                ...
                {canModify(debate) && (
                  <>
                    ...
                    <button onClick={() => handleDelete(debate.id)}
